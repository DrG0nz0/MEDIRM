...
namespace Scheduling
{
    ...
    {
        int NumberOfJobs, NumberOfProcesses, NumberOfMachines, NumberOfJobsXProcess = 0;
        bool refresh;
        Schedule[] population;
        ...
        {
            int[] jobSchedule = new int[NumberOfJobsXProcess];
            int[] macSchedule = new int[NumberOfJobsXProcess];
            int[] used = new int[NumberOfJobs];
            bool[] assigned = new bool[NumberOfJobsXProcess];
            List<int> rem = Enumerable.Range(0, NumberOfJobs).ToList();
            for (int j = 0; j < NumberOfJobsXProcess; j++)
            {
                ...
                if (rnd.Next(1, 101) <= minTimeOdd) //En küçük zamanı alarak popülasyonu iyileştir
                {
                    ...
                    int pos = selJob * NumberOfProcesses + used[selJob];
                ...
                {
                    if (!assigned[j])
                        macSchedule[j] = rnd.Next(0, NumberOfMachines);
                }
                used[selJob]++;
                if (used[selJob] == NumberOfProcesses)
                    rem.Remove(selJob);
            }

           return new Schedule(jobSchedule, macSchedule, NumberOfJobs, NumberOfProcesses, NumberOfMachines);
        }
        ...
        Schedule UniformCO(Schedule mother, Schedule father)
        {
            ...
            return new Schedule(jobs, macs, NumberOfJobs, NumberOfProcesses, NumberOfMachines);
        }
        #endregion
        ...
        public Schedule TwoPointCO(Schedule mother, Schedule father)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                while (used[mother.JobSchedule[k]] >= NumberOfProcesses)
                    k++;
            ...
            return new Schedule(gens, macAS, NumberOfJobs, NumberOfProcesses, NumberOfMachines);
        }
        #endregion
        ...
        public Schedule SinglePointCO(Schedule mother, Schedule father)
        {
            ...
            for (int i = x; i < N; i++)
            {
                ...
                while (used[father.JobSchedule[k]] >= NumberOfProcesses)
                    k++;
        ...
        Schedule OrderedCO(Schedule mother, Schedule father)
        {
            ...
            return new Schedule(jobOS, macAS, NumberOfJobs, NumberOfProcesses, NumberOfMachines);
        }
        #endregion
        ...
        void ChangeValueMutation(Schedule sch)
        {
            int N = NumberOfJobsXProcess;
            int x1 = rnd.Next(0, N);
            int mac = rnd.Next(0, NumberOfMachines);
        ...
        void ExchangeValuesMutation(Schedule sch)
        {
            ...
            x1 = rnd.Next(0, NumberOfJobsXProcess);
            x2 = rnd.Next(0, NumberOfJobsXProcess);
